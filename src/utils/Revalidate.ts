...
import { InvalidateCacheType } from '../types/InvalidateCacheType.js'
...
export const invalidateCache = ({ product, order, admin, userId, orderId, productId }: InvalidateCacheType) => {
 if (product) {
  const productCacheKeys: string[] = ["latest-product", "categories", "all-products"];
  if(typeof productId === "string") productCacheKeys.push(`product-${productId}`);
  if(typeof productId === "object") productId.forEach(productId => productCacheKeys.push(`product-${productId}`));
 ...
 if (order) {
  const orderCacheKeys: string[] = ["all-orders", `my-orders-${userId}`, `order-${orderId}`];
  // to delete the cache key of specific product we have extract the keys of individual products
