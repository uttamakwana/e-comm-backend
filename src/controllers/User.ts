...
// POST
//! Public
export const userRegister = TryCatch(async (req: Request<{}, {}, UserType>, res: Response, next: NextFunction) => {
 const { u_id, name, email, photo, gender, dob } = req.body;

 // user input all the information or not
 if (!u_id || !name || !email || !photo || !gender || !dob) return next(new ErrorHandler("Please input all the required fields!", 400))
