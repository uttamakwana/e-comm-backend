...
import { SearchProductQuery, FilterOptionsType } from '../types/Product.js';
...
export const getProduct = TryCatch(async (req, res, next) => {
 ...
 if (!ObjectId.isValid(id)) return next(new ErrorHandler("Provide a valid mongoDB id!", 400));
...
// routes: products/
// GET
export const getProducts = TryCatch(async (req: Request<{}, {}, {}, SearchProductQuery>, res, next) => {
...
const generateRandomFakeProducts = async (count: number = 10) => {
 const products = [];
...
export const createProduct = TryCatch(async (req: Request<{}, {}, ProductType>, res, next) => {
 const { name, price, stock, category } = req.body;
 // we are taking photo from the multer middleware named 'singleUpload'
 const photo = req.file
 // if any of one has not inputed
 if (!photo) return next(new ErrorHandler("Please add product image!", 400));
...
export const deleteProduct = TryCatch(async (req, res, next) => {
 ...
 if (!ObjectId.isValid(id)) return next(new ErrorHandler("Please provide a valid mongodb ID", 400));
