...
export const createCoupon = TryCatch(async (req, res, next) => {
 ...
 if (!code || !amount) return next(new ErrorHandler("Please enter all details!", 400));
...
export const applyDiscount = TryCatch(async (req, res, next) => {
 ...
 if (!coupon) return next(new ErrorHandler("Please give a valid coupon code!", 400));
 ...
 if (!discount) return next(new ErrorHandler("No coupon code found!", 404));
...
export const allCoupons = TryCatch(async (req, res, next) => {
 ...
 if (myCache.has("coupons")) {
  coupons = JSON.parse(myCache.get("coupons") as string);
 } else {
...
export const deleteCoupon = TryCatch(async (req, res, next) => {
 ...
 if (!code) return next(new ErrorHandler("Please enter valid coupon code!", 400));

 // find coupon
 const coupon = await CouponModel.findOne({ code });

 // error if not coupon
 if (!coupon) return next(new ErrorHandler("Coupon doesn't exist!", 404));
